...
from course.models import Student, User,Person
...
def register_user(request):
    ...
    if user_form.is_valid() and student_form.is_valid():
        ...
        if not email.endswith('@bic.edu.np'):
            messages.error(request,'Please enter the provided institutional email')
...
def login_stud(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            return render(request, 'login_stud.html', {'error_message': 'Invalid login credentials'})
